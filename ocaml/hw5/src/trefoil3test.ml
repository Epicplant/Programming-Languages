...
let%test "test binding parsing" =
  let parsed_test = bos "(test (= 250 x))" in
  let manually_constructed_test = Ast.TestBinding(Ast.Equals(Ast.IntLit(250), Ast.Variable("x"))) in
  parsed_test = manually_constructed_test

let%test "test binding parsing malformed" =
  try ignore (bos "(test (= 250))"); false
  with AbstractSyntaxError _ -> true
...
let%test "cond parsing test" =
  let parsed_cond = "(define (eval e)
  (cond
    ((= e 1) true)
    (true 'error)))
" in

  let manually_constructed_cond = Ast.BoolLit true in
  ieab0 (bsos (parsed_cond),
  eos "(eval 1)") = manually_constructed_cond
...
let%test "struct binding parsing" =
  let parsed_struct = bos "(struct my-record name age)" in
  let manually_constructed_struct = Ast.StructBinding({name = "my-record"; field_names = ["name"; "age"]}) in
...
let%test "match parsing test" =
  let parsed_match = eos "(match l (_ 25))" in
  let var = Ast.Variable("l") in
  let temp_list =  [(Ast.WildcardPattern, Ast.IntLit(25))] in
  ...
  let (manually_constructed_match : Ast.expr) = Ast.Match(var, temp_list)  in
  parsed_match = manually_constructed_match

  let%test "match parsing malformed" =
  try ignore (eos "(match 1 (33))"); false
  with AbstractSyntaxError _ -> true
...
let%test "match expression with bool literal patterns 2" =
  ...
  let%test "match expression with nil pattern" =
  let program = "(define x 3)" in
  Ast.BoolLit true = ieab0 (bsos program, eos "(match nil (nil true) )")
...
let%test "match struct binding" =
  ...
  Ast.Let("x", Ast.IntLit(3), Ast.Plus(Ast.Variable("x"), Ast.IntLit(1))) in
